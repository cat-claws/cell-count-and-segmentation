...
def get_hv_map(output,local_batch):
  # output: direct output from the model
  # local_batch: the input image
  instance = output.argmax(1).squeeze().cpu().numpy()
  target_dict = gen_targets(instance,[1000,1000])
  target_dict['img'] = local_batch.squeeze().cpu().numpy().reshape((1000,1000,3))
