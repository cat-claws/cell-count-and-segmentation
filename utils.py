...
def getEdgeMap(inst_map):
	borderExcluded = np.pad(np.ones_like(inst_map)[2:, 2:], 1).astype(bool)
	c = np.zeros_like(inst_map)
	for i in (-1, 1):        
		for j in (-1, 1):
			c[np.roll(inst_map, shift=(i, j), axis=(0, 1)) != inst_map] = 1
	return np.logical_and(c, borderExcluded)
